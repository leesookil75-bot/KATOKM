...
// Leaflet relies on window object, import dynamically
...
export default async function ParentShuttlePage() {
    ...
    // Fetch shuttle data server-side initially
    const host = headers().get('host');
    const protocol = process.env.NODE_ENV === 'development' ? 'http' : 'https';
    const res = await fetch(`${protocol}://${host}/api/parent/shuttle`, {
        headers: { Cookie: headers().get('cookie') || '' }
    });
    
    const data = await res.json();
    const shuttleInfo = data.assigned ? data.info : null;

    if (!shuttleInfo) {
        return (
            <div className="shuttle-page not-assigned">
                <header className="fixed-header">
                    <Link href="/parent" className="back-btn"><ChevronLeft size={24} /></Link>
                    <h1>실시간 셔틀 보기</h1>
                </header>
                <div className="empty-state">
                    <AlertCircle size={48} color="#94a3b8" />
                    <h2>배정된 노선이 없습니다</h2>
                    <p>학생이 어떤 노선과 정류장에 배정되어 있지 않습니다.<br/>학원에 문의해주세요.</p>
                    <Link href="/parent" className="home-btn">대시보드로 돌아가기</Link>
                </div>
            </div>
        );
    }
        ...
        <div className="shuttle-page">
            ...
            <div className="map-container-wrapper">
                <div className={`map-overlay ${shuttleInfo.is_driving ? 'driving' : 'standby'}`}>
                    {shuttleInfo.is_driving ? (
                        <div className="driving-badge">차량 운행 중</div>
                    ) : (
                        <div className="standby-curtain">
                            <Info size={40} />
                            <h2>셔틀 운행 대기 중입니다</h2>
                            <p>기사님이 운행을 시작하면<br/>지도를 통해 실시간 위치를 볼 수 있습니다.</p>
                        </div>
                    )}
                </div>
            ...
            <div className="info-panel">
                ...
                <div className="panel-row mt-2">
                    <span className="label">기사님 연락처</span>
                    <a href={`tel:${shuttleInfo.driver_phone}`} className="value phone-link">
                        {shuttleInfo.driver_phone}
                    </a>
                </div>
            </div>
            ...
            <style jsx>{`
                ...
                .standby-curtain p { font-size: 0.95rem; line-height: 1.5; margin: 0; }
                ...
                .phone-link { color: #2563eb; text-decoration: underline; font-weight: 600; }
                ...
                .home-btn { 
                    margin-top: 1rem; background: #f1f5f9; color: #475569; 
                    padding: 0.75rem 1.5rem; border-radius: 1rem; font-weight: 600; text-decoration: none;
                }
                .map-loading { height: 100%; display: flex; align-items: center; justify-content: center; color: #94a3b8; }
            `}</style>
        </div>
    );
}
